...
<section>
	...
	<c:choose> 
		...
		<div class="row left-gap">
			<input type="hidden" name="userid" id="userid" value="${user.userID}">
			 <div class="col-md-5">
			 	<div class="form-group">
					FirstName :<input type="text" name="firstname" maxlength="50" value="${formdata.firstname}${user.firstname}" id="firstname" class="form-control" placeholder="Enter First Name">
				</div>
				<div class="form-group">
					LastName :<input type="text" name="lastname" id="lastname" maxlength="50" value="${formdata.lastname}${user.lastname}" class="form-control" placeholder="Enter Last Name" required>
				 </div>
				<c:choose> 
				<c:when test="${user==null}">
				 <div class="form-group">
				 ...
				 <div class="form-group">
				   ...
				   <div class="radio">
				  	 <c:choose>
					      <c:when test="${user.gender=='Male'}${formdata.gender=='Male'}">
					  ...
					  <c:choose>
					      <c:when test="${user.gender=='Female'}${formdata.gender=='Female'}">
					  ...
					  <c:choose>
					      <c:when test="${user.gender=='Transgender'}${formdata.gender=='Transgender'}">
			...
			<div class="col-md-5">
				 <div class="form-group">
					Phone:<input type="text" name="phone" maxlength="10" value="${formdata.phone}${user.phone}" size="10" id="phone" class="form-control" placeholder="Enter Phone Number" required>
				</div>
				 <div class="form-group">
				 	Date of Birth: <input type="date" id="birthday" class="form-control" value="${formdata.dateofbirth}${user.dateofbirth}" name="birthdate" required>
				 </div>
				 <c:if test="${user==null}">
				  <div class="form-group">
				 	Gender:
				   <div class="radio">
				  	 <c:choose>
					      <c:when test="${formdata.gender=='Male'}">
							<label class="radio-inline"><input type="radio" name="Gender" value="Male" checked>Male</label>
						  </c:when>
						  <c:otherwise>
						  	<label class="radio-inline"><input type="radio" name="Gender" value="Male">Male</label>
					  ...
					  <c:choose>
					      <c:when test="${formdata.gender=='Female'}">
							<label class="radio-inline"><input type="radio" name="Gender" value="Female" checked>Female</label>
						  </c:when>
						  <c:otherwise>
						  	<label class="radio-inline"><input type="radio" name="Gender" value="Female">Female</label>
					  ...
					  <c:choose>
					      <c:when test="${formdata.gender=='Transgender'}">
							<label class="radio-inline"><input type="radio" name="Gender" value="Transgender" checked>Transgender</label>
						  </c:when>
						  <c:otherwise>
						  	<label class="radio-inline"><input type="radio" name="Gender" value="Transgender">Transgender</label>
				 ...
				 <div class="form-group">
				 	...
				 	<label class="checkbox-inline">
						 	<c:choose>
							      <c:when test="${fn:contains(user.language,'English') || fn:contains(formdata.language,'English')}">
						...
						<label class="checkbox-inline">
							<c:choose>
							      <c:when test="${fn:contains(user.language,'Hindi') || fn:contains(formdata.language,'Hindi')}">
						...
						<label class="checkbox-inline">
							<c:choose>
							      <c:when test="${fn:contains(user.language,'Gujarati') || fn:contains(formdata.language,'Gujarati')}">
						...
						<label class="checkbox-inline">
							<c:choose>
							      <c:when test="${fn:contains(user.language,'Chinese') || fn:contains(formdata.language,'Chinese')}">
			 		...
			 		<fieldset>
    					<legend>Security Questions:</legend>
			    			1.Who was your childhood super hero?<br>
			    				 <input type="text"  class="form-control" value="${formdata.answer1}" maxlength="60" name="q1" required><br>
			    			2.What was your childhood nickname?<br>
			    				 <input type="text" class="form-control" value="${formdata.answer2}" maxlength="60" name="q2" required><br>
    				</fieldset>
    			 </div>
    		 </div>
    	 </div>
...
<div id="main-container">
     <c:choose> 
		<c:when test="${user != null || formdata !=null}">
			 <c:choose> 
			<c:when test="${formdata !=null}">
				<c:forEach items="${formdata.address}" var='useradd'>
